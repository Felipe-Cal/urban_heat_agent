...
def generate_mock_data(
    ...
    if raw_temps:
        ...
        # Vectorized temperature adjustment and normalization
        # ⚡ Bolt Optimization: Replaced Python loop with NumPy vector operations (~30x faster)
        interp_temps += temp_variation
        fracs = np.clip((interp_temps - t_lo) / (t_hi - t_lo), 0.0, 1.0)
        norm_ts = 0.05 + 0.45 * fracs

        # Combine into result array (N x 3)
    ...
    try:
        ...
        process_assets(water, water_data, "WATER", "Water Body", "Water Resource", "rgba(14, 165, 233, 0.8)", 5000)
        process_assets(parks, park_data, "PARK", "Public Park", "Urban Park", "rgba(132, 204, 22, 0.8)", 5000)
        process_assets(green_roofs, green_roof_data, "ROOF", "Green Roof", "Eco Infrastructure", "rgba(163, 230, 53, 0.8)", 5000)
        process_assets(gardens, garden_data, "GARDEN", "Community Garden", "Bio-Asset", "rgba(77, 124, 15, 0.8)", 5000)
        process_assets(forests, forest_data, "FOREST", "Urban Forest", "Woodland", "rgba(21, 128, 61, 0.8)", 5000)
        process_assets(wetlands, wetland_data, "WETLAND", "Wetland", "Natural Marsh", "rgba(12, 74, 110, 0.8)", 5000)

        # Shelters — rich tag extraction (opening hours, capacity, AC, address)
        for element in shelters[:5000]:
            lat, lon = get_coords(element)
            if lat is None or lon is None:
                continue
            tags = element.get("tags", {})
